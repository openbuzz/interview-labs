...
// sessionBox renders the session facts box; the border tracks status and
// rows without a value (local sessions have no IP/region/size) are omitted.
func sessionBox(s *session.Session) string {
	...
	rows = appendRow(rows, "url", s.Meta.URL)
	rows = appendRow(rows, "password", s.Meta.GatewayPassword)
...
// printHandover renders the box, the raw ssh line for ready cloud sessions,
// and the NEXT block.
